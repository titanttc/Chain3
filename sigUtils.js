...
/**
 * Determines if the signature is valid from TTC sign
...
/**
 * Add the prefix to the message as identification from TTC system
 * if the signature is valid from TTC sign
 ...
 * 1-24: 'TTCNode Signed Message:' : actual header string
 * 25: 0x10 = '\n' : newline ASCII
...
var addPrefixToMessage = function addPrefixToMessage(message) {
  //Header: \x19TTCNode Signed Message:\n
 ...
 * then use `ecsign` to produce the same signature as the `mc_sign` from TTC nodes.
 * call for a given `message`, or fed to `ecrecover` along with a signature to recover the public key
 * For non-HEX string, to comply with TTC node sign process.
...
var signMcMessage = function signMcMessage(message, privateKey) {
  ...
  // add TTC sign header as TTC vnode before the 
  // "\x19TTCNode Signed Message:\n"
  var prefixMsg = addPrefixToMessage(toBuffer(message));
...
var signTransaction = function (tx, privateKey) {
  ...
  try {
    ...
    // for TTC, keep 9 fields instead of 6
    var vPos = 9;
