...
TTC JavaScript API was built for TTC chain. It was developed based on the Ethereum web3.js API routines and made some methods compatiable with the web3.js methods so the users can easily move their Ðapp to TTC chain. In
Chain3 also supported additional methods for TTC platform, such as VNODE and SCS methods.

To make a Ðapp work on TTC network, user should use the `chain3` object provided by the [chain3.js library](https://github.com/titanttc/chain3). It communicates to a local TTC  node through [JSON RPC](https://github.com/titanttc/TTC-core/wiki/JSON-RPC). chain3.js works with any TTC VNODE and SCS, which exposes an RPC layer.

`chain3` contains the `mc` object - `chain3.mc` (for specifically TTC Mother blockchain interactions), and the `scs` object - `chain3.scs` (for MicroChain interactions). Over time we'll introduce other objects for each of the other chain3 protocols. Working [examples can be found here](https://github.com/TTCChain/chain3/tree/master/example).
...
console.log(version); // "TTC/v0.1.0-develop/darwin-amd64/go1.9"
```
...
<h4 id="chain3versionnetTTC">chain3.version.TTC </h4>


    chain3.version.TTC
    // or async
    chain3.version.getTTC(callback(error, result){ ... })


##### Returns

`String` - The TTC protocol version.

##### Example

```js
var version = chain3.version.TTC;
...
```js
chain3.setProvider(new chain3.providers.HttpProvider('http://localhost:8545')); // 8545 for go/TTC
```
...
Will contain the current provider, if one is set. This can be used to check if TTC etc. set already a provider.
...
```js
// Check if TTC etc. already set a provider
...
```js
var str = chain3.sha3("Some ASCII string to be hashed in TTC");
...
```js
var str = chain3.toHex(TTC network);
console.log(str); // '0x6d6f6163206e6574776f726b'

console.log(chain3.toHex({TTC: 'test'}));
...
```js
var str = chain3.fromAscii('TTC network');
...
Converts a number of sha into the following TTC units:
...
2. `String` - One of the above TTC units.
...
Converts a TTC unit into sha. Possible units are:
...
Contains the TTC blockchain related methods.
...
You can read more about events [here](https://github.com/titanttc/wiki/wiki/TTC-Contract-ABI#example-javascript-usage).
...
see [namereg](https://github.com/titanttc/chain3.js/blob/master/example/namereg.html) example
...
//admin nodeInfo { id: 'a6f486af99679e00ec1a2bf77304e8c7f183987c8138a7c515a08ee42c5bebbda9f01474d43ba7176f891989dbdb78a6cbade67a941e6c2d5a83751039adba36',
  // name: 'TTC/v1.0.10-rc-89f6ab9c/darwin-amd64/go1.10',
...
//admin peers [ { id: '089554d6929600b9c70bbd6e1c12594697d0aec43127b9b29c6eb96faf06884fd284f56c3de64155d65e540b59a43f4fd07d8802b4b5e95b2922531e6096c2d5',
  //   name: 'TTC/v1.0.9-rc-c5e47f69/linux-amd64/go1.11',
  //   caps: [ 'mc/62', 'mc/63' ],
  //   network: 
