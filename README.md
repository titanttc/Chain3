# TTC JavaScript API version 0.1.22.

This is the [TTC](/TTC-core) compatible JavaScript API which implements the Generic JSON RPC spec as described in the Chain3.md. It's available on npm as a node module, for bower and component as an embeddable js and as a meteor.js package.
...
* v0.1.15 Added local TTC sign and verify signature methods.
...
* v0.1.8 Moved git repository to https://github.com/titanttc/chain3 and fixed some bugs.
* v0.1.7 Fixed uneven signature R & S.
* v0.1.6 A complete package with all tests.
* v0.1.4 First compatible version with TTC chain.

Some of the methods require running a local TTC node to use this library.
To use vnode methods, need to enable the vnode APIs in TTC node by:
--rpc --rpcport "8545" --rpcapi "chain3,mc,net,vnode"

To use scs methods, need to enable the SCS RPC ports by:
--rpc

More information is in Chain3.md or [TTC wiki](https://github.com/titanttc/TTC-core/wiki/Chain3).
...
Example codes to send TTC through signed transaction.
```js
var rawTx = {
      from: src.addr,
...
the TTC network using this API library.

### sign and verify the signatures

Example codes to sign a message using TTC network and verify the signature.

```js
  // Connect with the TTC network
  chain3.setProvider(new chain3.providers.HttpProvider('http://gateway.TTC.io/mainnet'));
  ...
  // Hash the message
  let sha3Msg = chain3.sha3("HELLO TTC!");
...
[repo]: https://github.com/titanttc/chain3
